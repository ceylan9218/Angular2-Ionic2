...
export class QuestionSlidePage {
  ...
  itemTapped(event, item) {
    if (!(item.value.selected)) {
        ...
        if (answeredQuestion.metadata.answersType == 'Single') {
            // selecting a new answer on a Single-type question
            // loop through each existing answer to question
            answeredQuestion.answers.map(
                function(ans) {
                    // set the answer to unselected
                    ans.value.selected = false;
                    // clear any primary remark inputs (shouldn't happen, but future-proofing
                    ans.value.remark = '';
                    // clear any optional inputs
                    ans.optionalInput.remark = '';
                }
            );
        }
  ...
  saveTapped(event) {
    ...
    // determine if an answer is required for this question
    let requireAnswer = (answeredQuestion.metadata.required == true);
    
    // save the current screen response(s)
    ...
    // check if a JSON object was built
    if (params) {
        console.log("Saving user answer to question and proceeding to submit page.");
        // new answer data exists for this page
        // so save answer via call to AnswerService
        this.answerService.addAnswer(params)
                .subscribe(
                  data => {
                      this.data = data
      ...
      // check if it is okay to proceed without any answer
      if (!(requireAnswer)) {
        console.log("No answer given but proceeding to submit anyway.");
        // no new data to send - just proceed to "submit concern" page
        this.nav.push(SubmitConcernPage, {
            page: this,
            question: this.question
        });
      } else {
        // can't proceed - need an answer first
        // TODO: remove this debug block
        console.log("Cannot submit until answer given.");
      }
    }
  }
